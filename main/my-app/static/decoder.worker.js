...
Decoder.prototype.sendData = function (data) {
  ...
  const res = Module._sendData(this.cacheBuffer, typedArray.length);
...
Decoder.prototype.processReq = function (req) {
  ...
  switch (req.k) {
    case kInitDecoderReq:
      this.initDecoder(req.data.size, req.data.chunkSize);
      break;
    ...
    case kPauseDecodingReq:
      ...
      break;
    case kFeedDataReq:
      this.sendData(req.data.raw_data);
      break;
    case kSeekToReq:
      this.seekTo(req.ms);
      break;
